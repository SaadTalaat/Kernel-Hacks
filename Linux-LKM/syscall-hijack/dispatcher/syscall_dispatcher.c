...
chdir_init(void){
	...
	/* get idt via native store idt
	 * since normal asm call will
	 * give obfuscated value, don't ask me why.
	 */
	store_idt(&idtr);
	...
	*((unsigned int *) &syscall_handler) = syscall_disp;
	...
	new_syscall->a = (orig_syscall->a & 0xFFFF0000); //copy segment selector
	new_syscall->b = (orig_syscall->b & 0x0000FFFF); //copy flags
